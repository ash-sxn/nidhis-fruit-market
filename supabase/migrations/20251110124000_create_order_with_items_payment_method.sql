...
declare
  ...
  v_variant_id uuid;
  v_product_uuid uuid;
  v_payment_method text := case when lower(coalesce(p_payment_method,'online')) = 'cod' then 'cod' else 'online' end;
begin
  v_user_id := auth.uid();
...
grant execute on function public.create_order_with_items(text, jsonb, jsonb, integer, text, text, text)
  to authenticated;
